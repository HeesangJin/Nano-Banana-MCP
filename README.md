...
A Model Context Protocol (MCP) server that provides AI image generation and editing capabilities using Google's Gemini image generation API. Generate stunning images, edit existing ones, and iterate on your creations with simple text prompts.
...
- `GEMINI_IMAGE_MODEL` is optional.
- If not set, the server tries built-in fallback models automatically.
- If set, your configured model is tried first.
...
1. **🥇 MCP Configuration Environment Variables** (Highest Priority)
   ...
   - Example: `"env": { "GEMINI_API_KEY": "your-key", "GEMINI_IMAGE_MODEL": "gemini-2.0-flash-preview-image-generation" }`

2. **🥈 System Environment Variables**
   - Set in your shell/system environment
   - Example: `export GEMINI_API_KEY="your-key"` and `export GEMINI_IMAGE_MODEL="gemini-2.0-flash-preview-image-generation"`
...
- **Google AI** - For the Gemini image generation API
